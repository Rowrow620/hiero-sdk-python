...
def setup_client():
    ...
    try:
        ...
        # Reverting to the original key loading mechanism
...
def generate_fallback_key() -> PrivateKey:
    """Generate a single ECDSA key pair for the account and return the key."""
    print("\nSTEP 1: Generating a single ECDSA key pair for the account...")
    
    # Using the updated, explicit generation method
    ...
    if evm_address is None:
        raise RuntimeError("Failed to generate EVM address from ECDSA public key.")
...
def create_account_with_fallback_alias(client: Client, account_private_key: PrivateKey) -> AccountId:
    """Creates the account using the key/alias fallback behavior and returns the new AccountId."""
...
def fetch_account_info(client: Client, account_id: AccountId):
    """Fetches and returns the account info."""
    account_info = (
        AccountInfoQuery()
...
def print_account_summary(account_info) -> None:
    """Prints the account info summary."""
...
def main():
    """Main entry point: orchestrates the account creation workflow."""
    try:
        client = setup_client()
